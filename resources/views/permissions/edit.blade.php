                ...
                <div class="col-md-8">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form method="POST" action="{{ route('permissions.update', $permission) }}" class="mt-3 mb-3">
                                @csrf
                                @method('patch')
                                <div class="mb-3">
                                    <x-input-label for="name" :value="__('Name')" />
                                    <x-text-input id="name" name="name" type="text" class="mt-1"
                                        :value="old('name', $permission->name)" required autocomplete="name" />
                                    <x-input-error class="mt-2" :messages="$errors->get('name')" />
                                </div>
                                <div class="mb-3">
                                    <x-input-label for="description" :value="__('Description')" />
                                    <x-text-input id="description" name="description" type="text" class="mt-1"
                                        :value="old('description', $permission->description)" required autocomplete="description" />
